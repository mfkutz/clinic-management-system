...
**Corrección post-feedback del usuario**: al probar el flujo en vivo, notó que pedirle al paciente "confirmar asistencia" apenas termina de reservar es un paso redundante — en este sistema la reserva es siempre self-service (no hay recepción que reserve por el paciente y necesite que él reconfirme después), así que el acto de reservar *ya es* la confirmación. Se corrigió `appointmentService.createAppointment` para crear el turno con `confirmedByClient: true` directamente (antes quedaba en `false`, el default de la columna, y el paciente tenía que confirmar de nuevo algo que acababa de elegir). El endpoint `/confirm` y todo el UI de "Sin confirmar" se mantienen intactos — siguen siendo reales y se siguen pudiendo demostrar con la mezcla que dejé en el seed (turnos ya existentes con `confirmedByClient: false`), pero un turno nuevo reservado desde la app ya no pasa por ese estado.
...
## Pasada de cierre pre-capturas: Pacientes, Cobros y rol Profesional

El usuario va a subir el proyecto a Workana y quiere sacar capturas para el portfolio ya, completando el resto más adelante. Pedido explícito: no dejar a la vista ningún placeholder obvio (mapa vacío, pantallas sin el lenguaje visual nuevo) que se note en una foto estática — no se trata de que todo sea 100% funcional, sino de que nada se vea "sin terminar" a simple vista. De acá salió también el mapa real de la sección anterior. Esta pasada cubre las 3 piezas que quedaban con el estilo Tailwind genérico viejo, sin handoff de Claude Design (mismo criterio que Reportes: diseñado de cero con los tokens/helpers ya establecidos).

- **`PatientsPage.tsx` (Pacientes)**: header + KPI strip (pacientes totales/activos/inactivos/turnos acumulados, reales) + buscador + lista en cards (antes era un `<table>` con clases Tailwind default). Reusa `avatarStyle`/`getInitials`. Agregada a `SHARED_FULL_BLEED_PATHS` en `Layout.tsx` (la usan admin y profesional). Sin cambios de datos/lógica, solo visual.
- **`BillingPage.tsx` (Cobros)**: mismo criterio — KPI strip (cobrado/pendiente/% cobrado con barra), tabs Todos/Pendientes/Cobrados con conteos reales, buscador por paciente/servicio/profesional, filas con avatar y badge de estado. La lógica de "marcar cobrado" (select de método + PATCH real) es exactamente la misma que ya existía, solo se restyleó. Agregada a `ADMIN_ONLY_FULL_BLEED_PATHS`.
- **Rol Profesional, Inicio** (`pages/professional/ProfessionalHomePage.tsx`, nuevo): hasta ahora `HomePage.tsx` mostraba un saludo genérico sin dashboard para este rol — se armó uno con el mismo lenguaje visual que `AdminHomePage`/`ClientHomePage`: KPIs (turnos hoy, atendidos hoy, ausentismo 7 días, turnos de la semana), agenda de hoy (mismo patrón visual que la de Inicio-admin pero sin los widgets que no aplican a un profesional individual — sin selector de rango ni datos cruzados de otros profesionales), gráfico de turnos de la semana (mismo componente visual, reimplementado localmente porque no se extrajo a un helper compartido — recién sería la 2ª vez que se repite, no la 3ª), accesos rápidos, y un widget real de "Turnos sin confirmar" (sus propios turnos próximos con `confirmedByClient: false`, informativo — el profesional no tiene una acción de confirmar en nombre del paciente).
- **`ProfessionalAvailabilityPage.tsx` (Disponibilidad)**: mismo formulario y misma lógica (`useForm`+Zod, altas/bajas de disponibilidad y excepciones sin tocar), pero restyleada — la disponibilidad semanal ahora se agrupa visualmente por día (antes era una lista plana "Lunes: 09:00–13:00") con cada franja como chip removible, y las excepciones como cards con ícono según si bloquean o agregan horario.
- `Layout.tsx`: se simplificó la condición de full-bleed de `/inicio` — ahora es full-bleed para cualquier rol autenticado (ya no hace falta la lista `HOME_FULL_BLEED_ROLES`, porque las 3 versiones de Inicio están rediseñadas). Se agregó `/profesional/disponibilidad` a un nuevo `PROFESSIONAL_ONLY_FULL_BLEED_PATHS`.
- **Detalle a tener en cuenta (no es un bug de esta pasada, es preexistente)**: `PatientSummary.lastVisit` (usado en la lista de Pacientes) es el `MAX(start_datetime)` de *todos* los turnos del paciente, no solo los `completed` — puede mostrar una fecha futura si el próximo turno confirmado es más reciente que la última visita real. `PatientDetailPage` sí calcula bien "última visita" (solo turnos `completed`). No se tocó `patientService.ts` en esta pasada porque el pedido era visual, pero si se quiere consistencia real valdría la pena unificar el criterio ahí.
- Verificado con Playwright: Inicio/Disponibilidad como profesional, Pacientes como admin y como profesional (confirmando que el scoping por rol —cada profesional solo ve a los pacientes que atendió— sigue funcionando, no es un bug nuevo que "Valentina" vea a los 14 si efectivamente atendió a los 14 en el dataset demo), Cobros como admin, y navegación Pacientes → detalle de paciente. `tsc -b` y `oxlint` limpios, cero errores de consola.

## Pasada de responsive (mobile)

Todo el rediseño se había hecho pensando solo en desktop (sidebar fijo de 264px, grids con columnas en `fr`/px fijos). El usuario probó en el celular y "se rompía todo" — esta pasada cubrió el shell y las ~20 pantallas, con foco en admin/profesional/cliente y los 3 flujos completos (reserva, mis turnos, agenda).

**Shell** (`Layout.tsx`, `Sidebar.tsx`, `TopHeader.tsx`): el sidebar pasó a ser un drawer off-canvas en `<lg` (oculto por `-translate-x-full`, se abre con un hamburger nuevo en el header, con backdrop y botón de cierre propio) en vez de compartir el mismo estado que el colapsar-a-íconos de desktop — antes el botón hamburger de mobile literalmente llamaba al mismo `onToggle` del modo icon-rail de desktop, que no sirve como menú mobile. `TopHeader` esconde el buscador (`md:`), los íconos decorativos (ahora `lg:`, antes `sm:`) y el label del botón primario (`sm:`) para no competir por espacio en una pantalla angosta.

**Criterio general aplicado pantalla por pantalla**: grids de KPIs `grid-cols-4/5` → `grid-cols-2` en mobile; splits de dos columnas (`grid-cols-[1.7fr_1fr]` etc.) → `grid-cols-1` con `lg:` para volver al split; padding de página `px-[28px]` → `px-4` con `lg:` para el valor original; filas de lista con muchas columnas de ancho fijo (Pacientes, Cobros) → se ocultan columnas secundarias bajo `sm:` y se apila lo esencial.

**Bug real encontrado y corregido — comillas en `items-center` + texto que puede wrappear**: en `PatientDetailPage.tsx`, la fila de "Historial de turnos" tenía `items-center` con el bloque de info (`flex-1 min-w-0`, sin `truncate` en la línea de profesional/especialidad) al lado de la fecha y el badge de estado (`shrink-0`). Cuando el nombre del servicio wrappeaba a 2 líneas en una pantalla angosta, `items-center` centraba verticalmente los elementos `shrink-0` contra el bloque más alto, haciendo que la fecha quedara pisando la 2ª línea del texto. Se resolvió apilando la fila en mobile (`flex-col` → `sm:flex-row sm:items-start`) — `items-start` en vez de `items-center` evita que vuelva a pasar aunque el texto crezca.

**Trampa de medición — por qué usar `document.documentElement.scrollWidth` no alcanza**: `<main>` en `Layout.tsx` tiene `overflow-y-auto` sin `overflow-x` explícito. Por la regla de cómputo de la spec de CSS Overflow, el navegador fuerza el `overflow-x` a `auto` también (no puede quedar en `visible` si el otro eje no lo es) — así que cualquier overflow horizontal dentro de `<main>` queda contenido y scrolleable *ahí*, sin nunca propagarse a `document.documentElement`. Verificar overflow mobile chequeando solo `document.documentElement.scrollWidth > clientWidth` da falso negativo en toda la app. Esto hizo que dos bugs reales pasaran desapercibidos en una primera pasada de verificación:
- **`BillingPage.tsx`** (Cobros): la fila de cada turno tenía columnas de ancho fijo (110+100+140+210px) sin `flex-wrap`, sumando ~700px — se resolvió con el mismo patrón de apilado responsive que las demás filas de lista.
- **`AdminProfessionalsPage.tsx` / `AdminServicesPage.tsx`**: la grilla de cards en vista "grid" usa `repeat(auto-fill, minmax(400px,1fr))` / `minmax(340px,1fr)` — que en teoría "colapsa a 1 columna" en pantallas angostas, pero en la práctica el navegador no reduce el mínimo de `minmax()` por debajo del valor fijo aunque el contenedor sea más chico, así que la card se renderiza a 400/340px igual y desborda. Se corrigió con `minmax(min(400px,100%),1fr)` (clampea el mínimo al 100% del contenedor disponible), el fix estándar para este gotcha de CSS Grid.
- **`MyAppointmentsPage.tsx`**: las tabs (Próximos/Historial/Todos) no tenían `flex-wrap` ni scroll — a 320px de ancho el texto+badge de las 3 tabs no entraba. Se le agregó `overflow-x-auto` (scroll horizontal intencional, no el bug de arriba) y se redujo padding/tamaño de fuente en mobile.

**Verificación**: en vez de mirar screenshots nomás, se escribió un script de Playwright que mide `main.scrollWidth - main.clientWidth` (la medición correcta) en cada ruta, para los 3 roles, a 375px y 320px de ancho (iPhone SE, el piso realista), incluyendo estados que solo aparecen tras interactuar (tabs cambiadas, drawers abiertos, los 5 pasos del wizard de reserva, una reserva real de punta a punta). Confirmado en cero en todos los casos después de las correcciones. `tsc -b` y `oxlint` limpios.
